...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
  var totalAmount float64 = calcPrice(productID, quantity)

  if amountInStock(productID) < quantity || accountBalance < totalAmount {
    return false, accountBalance
  }

  return true, accountBalance - totalAmount
}
